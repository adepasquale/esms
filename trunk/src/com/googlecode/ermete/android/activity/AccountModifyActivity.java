...
public class AccountModifyActivity extends Activity {
	...
	final Account account = (Account) intent.getSerializableExtra(
		"com.googlecode.ermete.ACCOUNT");

	titleLogo.setImageBitmap(BitmapFactory.decodeResource(
		getResources(), account.getLogoID()));

	titleText.setText(
		getString(R.string.account_modify_activity) + 
		" " + account.getLabel());
	...
	if (action.equals("com.googlecode.ermete.CHOOSE_LABEL")) {
		...
		public void onTextChanged(
			CharSequence s, int start, int before, int count) {
		    if (s.length() > 0) nextButton.setEnabled(true);
